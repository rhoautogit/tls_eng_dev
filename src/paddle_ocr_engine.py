...
# PaddlePaddle GPU memory management: use auto_growth allocator so VRAM
# is allocated on demand and empty_cache() can actually release it.
# Without this, PaddlePaddle pre-allocates a large memory pool that
# never shrinks, causing OOM on long-running scanned PDF batches.
...
class PaddleOCREngine:
    ...
    def release_shared(cls) -> None:
        """Destroy the shared singleton and release all GPU memory.

        Call this after extraction is complete and before Qwen-VL
        verification so Ollama can use the full GPU.
        """
        import gc
        if cls._shared_instance is not None:
            inst = cls._shared_instance
            # Clear internal PaddleOCR predictor references
            if inst._ocr is not None:
                try:
                    for attr in ("text_detector", "text_recognizer",
                                 "text_classifier", "page_num"):
                        if hasattr(inst._ocr, attr):
                            setattr(inst._ocr, attr, None)
                except Exception:
                    pass
            inst._ocr = None
            inst._table_engine = None
            ...
            try:
                ...
                pass
            logger.info("PaddleOCR engine released, GPU memory freed")
    ...
    _subprocess_batch: List[Tuple[str, np.ndarray]] = []
    _subprocess_results: Dict[str, OCRPageResult] = {}
    _subprocess_batch_size: int = 10

    def ocr_image_subprocess(self, image: np.ndarray) -> OCRPageResult:
        """Run OCR in a subprocess for guaranteed GPU memory release.

        Images are batched: accumulated until batch_size is reached, then
        all are OCR'd in a single subprocess. This amortizes model loading
        cost (~3s) across multiple pages instead of paying it per-page.
        """
        ...
        img_file = tempfile.NamedTemporaryFile(
            suffix=".png", delete=False, prefix="paddle_img_"
        )
        cv2.imwrite(img_file.name, image)
        img_path = img_file.name
        img_file.close()

        # Release in-process engine to free GPU before subprocess loads its own
        PaddleOCREngine.release_shared()

        try:
            results = run_paddleocr_subprocess([img_path], self.config)
            return results.get(img_path, OCRPageResult())
        finally:
            try:
...
def run_paddleocr_subprocess(
    ...
    try:
        ...
        if proc.returncode != 0:
            logger.error("PaddleOCR subprocess failed (rc=%d): %s",
        ...
        try:
            ...
            logger.error("PaddleOCR subprocess output unreadable: %s", e)
            return {}
